...
spec = yaml.safe_load((root / 'openapi/archive/club-platform-0.6-g0.yaml').read_text(encoding='utf-8'))
validate(spec)
...
current = yaml.safe_load((root / 'openapi/club-platform.yaml').read_text())
validate(current)
assert current['info']['version'] == '1.0.0-pilot'
assert '/api/v1/auth/me' in current['paths']
print('OK: historical G0 contract, current pilot authentication contract and ABI-2 compatibility example.')
