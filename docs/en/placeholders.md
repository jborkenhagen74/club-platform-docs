...
Baseline: schema 18 / extension ABI V3. Extracted from actual core registration and shipped module manifests. Use exact tokens such as `{{person.full_name}}`, with no inner spaces. Availability also depends on installation data and permissions.

`records.read` is always required. Modules also require their read permission and valid licensing, installation and activation. `membership_id` supplies person and organization. Multiple matching addresses, emails, phone numbers, positions or module records require explicit `address_id`, `email_id`, `phone_id`, `position_id` or `extension_record_id`; ambiguous contexts are rejected.

`document.datetime` currently returns the document date plus `T00:00:00`, not the current time. Dates use ISO representation; `raw` returns unformatted text. `person.age` is calculated at the document date. Missing optional values may be empty; missing permission is an error.
...
## Dynamic fields and limits

- `{{person.custom.<key>}}` and `{{organization.custom.<key>}}`: one token per configured custom field; type follows the field definition and field-read permission is required. `<key>` is a pattern, not a literal token.
- Legacy aliases: `{{given_name}}` → `person.given_name`, `{{family_name}}` → `person.family_name`, `{{date}}` → `document.date`.
- Other installed modules may register additional tokens. Query the authorized live catalogue: `GET /api/v1/documents/placeholders?scope=person` (also `global`, `organization`, `membership`).
- Finance, Contributions, Purchases, Calendar, Events, Banking and athlete licenses currently register no document placeholders. Planned tokens are not listed as available.
